...
//  Street 1 : z = -100, x = -80 → +80   (east–west connector)
//  Street 2 : z =  300, x = -80 → +80   (east–west connector)
//  Bridge   : z = +100, x = -80 → +80   (elevated section)
...
function makeDashes(
  ...
  for (let v = from; v < to; v += gap) {
    const d = new THREE.Mesh(
      new THREE.PlaneGeometry(axis === 'z' ? 0.2 : 5, axis === 'z' ? 5 : 0.2),
      mat
    );
    d.rotation.x = -Math.PI / 2;
    d.position.set(
      axis === 'z' ? fixed + offset : v + gap / 2,
      y,
      axis === 'z' ? v + gap / 2 : fixed + offset
    );
    scene.add(d);
  }
}
...
const GameScene = () => {
  ...
  // FIX: car position stored in state (not ref) so render can safely read it
  ...
  useEffect(() => {
    ...
    // FIX: capture mount node at effect start so cleanup ref stays valid
    ...
    const YBR = 0.02; // road on bridge Y offset

    // ════════════════════════════════════════════════════
    //  1. HIGHWAY  (x=0, z=-1000→+200)
    // ════════════════════════════════════════════════════
    makeRoad(scene, 10, 1200, 0, Y, -400);
    ...
    for (let sz = -900; sz < 200; sz += 200) {
      [-6, 6].forEach((sx) => {
        ...
        const sign = new THREE.Mesh(
          new THREE.BoxGeometry(3, 0.9, 0.12),
          signMat
        );
    ...
    // ════════════════════════════════════════════════════
    //  6. BRIDGE  (elevated, z=+100, x=-80→+80)
    // ════════════════════════════════════════════════════
    // Ramp left
    const rampL = new THREE.Mesh(
    ...
    const deck = new THREE.Mesh(
      new THREE.BoxGeometry(deckW, 0.5, 10),
      new THREE.MeshStandardMaterial({ color: BRIDGE_COLOR })
    );
    ...
    [-28, -14, 0, 14, 28].forEach((px) => {
      const pillar = new THREE.Mesh(
        new THREE.CylinderGeometry(0.6, 0.8, YB, 10),
        pillarMat
      );
    ...
    [-4.2, 4.2].forEach((rz) => {
      const rail = new THREE.Mesh(
        new THREE.BoxGeometry(deckW, 0.7, 0.15),
        railMat
      );
    ...
    // ════════════════════════════════════════════════════
    //  7. TURNS  (90° arcs connecting highway ↔ streets)
    // ════════════════════════════════════════════════════
    const TURN_R = 12;
    ...
    (
      [
        ...
        { x: 64.5, z: 100, w: 0.4, d: 10 },
      ] as { x: number; z: number; w: number; d: number }[]
    ).forEach(({ x, z, w, d }) => {
    ...
    const addTree = (tx: number, tz: number, scale: number) => {
      ...
      [
        { r: 1.6 * scale, h: 2.2 * scale, cy: 2.5 * scale },
        { r: 1.2 * scale, h: 1.8 * scale, cy: 3.8 * scale },
        { r: 0.7 * scale, h: 1.4 * scale, cy: 4.8 * scale },
      ].forEach(({ r, h, cy }) => {
    ...
    const addObstacle = (ox: number, oz: number, oy = 0.5) => {
      const obs = new THREE.Mesh(new THREE.BoxGeometry(1.2, 1.0, 1.2), obsMat);
      obs.position.set(ox, oy, oz);
      obs.castShadow = true;
    ...
    for (let i = -800; i < 200; i += 45 + Math.random() * 25)
      [-3.5, 3.5].forEach((s) =>
        addObstacle(s + (Math.random() - 0.5) * 1.5, i)
      );
    for (let oz = -150; oz < 550; oz += 60 + Math.random() * 30)
      addObstacle(-60 + (Math.random() - 0.5) * 3, oz);
    for (let oz = -150; oz < 550; oz += 60 + Math.random() * 30)
      addObstacle(60 + (Math.random() - 0.5) * 3, oz);
    for (let ox = -60; ox < 60; ox += 40 + Math.random() * 20)
      addObstacle(ox, -100 + (Math.random() - 0.5) * 2);
    for (let ox = -60; ox < 60; ox += 40 + Math.random() * 20)
      addObstacle(ox, 300 + (Math.random() - 0.5) * 2);
    ...
    const crashState = { current: false };
    ...
    const getRoadZone = (px: number, pz: number): string => {
      if (Math.abs(px) < 5 && pz > -1000 && pz < 250) return 'HIGHWAY';
      if (Math.abs(px - -60) < 4 && pz > -200 && pz < 600) return 'AVENUE A';
      if (Math.abs(px - 60) < 4 && pz > -200 && pz < 600) return 'AVENUE B';
      if (Math.abs(pz - -100) < 5 && Math.abs(px) < 80) return 'STREET 1';
      if (Math.abs(pz - 300) < 5 && Math.abs(px) < 80) return 'STREET 2';
      if (Math.abs(pz - 100) < 6 && Math.abs(px) < 44) return '🌉 BRIDGE';
      return 'OFF ROAD';
    };
    ...
    const animate = () => {
      ...
      if (keys.reset) {
        ...
        speed = 0;
        g.speed = 0;
        crashState.current = false;
        setCrash(false);
        keys.reset = false;
      }

      if (!crashState.current) {
      ...
      if (g.car && g.camera) {
        ...
        // FIX: update React state (not ref) so mini-map dot re-renders correctly
        if (frameCount++ % 30 === 0) {
          const p = g.car.position;
          setRoadInfo(getRoadZone(p.x, p.z));
          setCarMapPos({ x: p.x, z: p.z });
        }
      }
    ...
    return () => {
      ...
      // FIX: use captured mountNode variable, not mountRef.current
  ...
  // FIX: toMap is a pure function — safe to call during render
  const toMap = (x: number, z: number) => ({
    mx: MAP_CX + x * MAP_SCALE,
    my: MAP_CY + z * MAP_SCALE,
  });

  // FIX: read from state (not ref) — safe during render
    ...
    >
      ...
      {crash && (
        ...
        >
          ...
          >
            Press{' '}
            <span style={{ color: '#ffdd88', fontWeight: 'bold' }}>R</span> to
            restart
          </div>
        </div>
      )}
